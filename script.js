...
function markFieldError(id, hasError) {
  ...
  if (!el) return;
  el.classList.toggle('auth-input-error', !!hasError);
}

['auth-email', 'auth-password'].forEach((id) => {
  document.getElementById(id)?.addEventListener('input', () => markFieldError(id, false));
});
...
async function handleLogin(event) {
  ...
  markFieldError('auth-email', false);
  markFieldError('auth-password', false);

  if (!email || !password) {
    if (!email) markFieldError('auth-email', true);
    if (!password) markFieldError('auth-password', true);
